...
public static class AppsStage
{
	...
	public static List<(string Title, Func<Task> Action, Func<bool>? Condition)> GetActions(IStatusReporter reporter = null!, ApplicationSelection selection = null!)
	{
		...
		var actions = new List<(string Title, Func<Task> Action, Func<bool>? Condition)>
		{
			...
			("Optimizing Photos settings", async () => await StoreHelper.KillProcesses("Microsoft.Windows.Photos_8wekyb3d8bbwe"), () => selection == null),
			("Optimizing Photos settings", async () => await Process.Start(new ProcessStartInfo { FileName = "reg.exe", Arguments = $@"load HKU\TEMP ""{Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "Packages", "Microsoft.Windows.Photos_8wekyb3d8bbwe", "Settings", "settings.dat")}""", CreateNoWindow = true })!.WaitForExitAsync(), () => selection == null),
			("Optimizing Photos settings", async () => RegistryHelper.SetValue(RegistryHelper.Identity.CurrentUser, @"HKEY_USERS\TEMP\LocalState", "AppBackgroundRequestedTheme", RegistryHelper.ApplicationDataBool, [0x00, 0x00, 0x00, 0x00, 0x53, 0xc1, 0xd4, 0x68, 0x02, 0x29, 0xdd, 0x01]), () => selection == null),
			("Optimizing Photos settings", async () => await Process.Start(new ProcessStartInfo { FileName = "reg.exe", Arguments = @"unload HKU\TEMP", CreateNoWindow = true })!.WaitForExitAsync(), () => selection == null),
			("Optimizing Photos settings", async () => { try { FileSystem.CopyDirectory(Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "Packages", "Microsoft.Windows.Photos_8wekyb3d8bbwe"), Path.Combine(Path.GetPathRoot(Environment.SystemDirectory)!, "Users", "Default", "AppData", "Local", "Packages", "Microsoft.Windows.Photos_8wekyb3d8bbwe"), true); } catch { } }, () => selection == null),
			...
			("Installing Windhawk", async () => await Process.Start(new ProcessStartInfo { FileName = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ProgramFiles), "Windhawk", "windhawk-cli.exe"), Arguments = @$"data import ""{Path.Combine(Path.GetTempPath(), "windhawk.json")}"" --confirm-app-restart --yes", WorkingDirectory = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ProgramFiles), "Windhawk"), WindowStyle = ProcessWindowStyle.Hidden })!.WaitForExitAsync(), () => selection == null),
			...
			("Installing Windhawk", async () => RegistryHelper.SetValue(RegistryHelper.Identity.TrustedInstaller, @"HKEY_LOCAL_MACHINE\SOFTWARE\Windhawk\Engine\Mods\auto-theme-switcher", "Disabled", 1, RegistryValueKind.DWord), () => ScheduleMode is "Always Light" or "Always Dark"),
			...
			("Setting appearance to system", async () => Process.Start(new ProcessStartInfo { FileName = Path.Combine(Directory.GetDirectories(Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "Discord"), "app-*").FirstOrDefault() ?? "", "Discord.exe"), WindowStyle = ProcessWindowStyle.Hidden }), () => Discord == true),
			("Setting appearance to system", async () => await Task.Delay(5000), () => Discord == true),
			...
			("Installing Vanguard", async () => RegistryHelper.SetValue(RegistryHelper.Identity.TrustedInstaller, @"HKEY_LOCAL_MACHINE\SYSTEM\CurrentControlSet\Services\vgk", "Start", 3, RegistryValueKind.DWord), () => RiotClient == true),
			...
			// remove hidhide desktop shortcut
			("Removing HidHide desktop shortcut", async () => File.Delete(Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.CommonDesktopDirectory), "HidHide Configuration Client.lnk")), () => HidHide == true),
		...
		// download raceelement
			...
			// install zed
			("Installing Zed", async () => await Process.Start(new ProcessStartInfo { FileName = Path.Combine(Path.GetTempPath(), "Zed-x86_64.exe"), Arguments = "/VERYSILENT /NORESTART /MERGETASKS=!runcode" , WindowStyle = ProcessWindowStyle.Hidden })!.WaitForExitAsync(), () => Zed == true),
